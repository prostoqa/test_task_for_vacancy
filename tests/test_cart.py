...
def test_add_item_to_cart_and_delete(driver):
    ...
    assert cart_on_main_page == ("1", item_price), "Wrong item data on the main page cart after adding to cart"
    main.go_to_cart_from_main_page()
    cart_page = cart.check_cart_page()
    assert cart_page == ("Корзина:", item_name, item_price, "1"), "Wrong item data on the cart page after adding to cart"
    cart.delete_item_from_cart()
    cart_on_main_page = main.get_items_and_price_on_empty_cart()
    assert cart_on_main_page, "Wrong item data on the main page cart after deleting from cart"
    main.go_to_cart_from_main_page()
    assert cart.get_empty_cart_title() == "Корзина пуста", "Cart is not empty"
